...
void initStorage(Graph *graph)
{
    ...
    graph->storage.pruneBuffer->data = hnsw_alloc_mem(sizeof(heapItem) * (graph->Mmax0), alignof(heapItem));
    HNSW_ASSERT(graph->storage.pruneBuffer->data);
    graph->storage.pruneBuffer->size = 0;
...
void makeNode(node *node, vec v, uint32 id, uint32 nodeLevel, uint32 maxNeigbours, int32 mMax0)
{
    ...
    node->neigbours = hnsw_alloc_mem(sizeof(uint32) * ((mMax0 + maxNeigbours) * allocationLevel), alignof(uint32));
