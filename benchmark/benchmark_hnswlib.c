...
void benchmark_insertion_hnswlib(int id, const char* lib_path)
{
    char base_path[256];
    ...
    snprintf(filename, sizeof(filename), "hnswLib_benchmark_insertion_%d_%s.csv", id, lib_path);
    ...
    {
        HnswlibIndex* index = hnswlib_init(ds.dim, ds.n,M,ef);
...
void benchmark_searchhnswLib(int id, const char *dataset_path)
{
    ...
    char filename[500]; // make sure it's big enough
    snprintf(filename, sizeof(filename), "hnswlib_benchmark_search_%d_%s.csv", id, dataset_path);
    replace_str(filename, '/', '_');
    ...
    HnswlibIndex* index = hnswlib_init(baseV.dim, maxElements, M,ef);
    ...
    HnswlibBFIndex* bf = hnswlib_bf_init(baseV.dim, baseV.n);

    // build brute-force index
    for (int i = 0; i < baseV.n; i++) {
    float *v = baseV.data + i * baseV.dim;
    hnswlib_bf_add(bf, v, i);
    }

    // compute GT
    for (int i = 0; i < qv.n; i++) {
    float *q = qv.data + i * qv.dim;
    hnswlib_bf_search(bf, q, K, groundtruth + i * K);
    }
    ...
    for (int e = 0; e < num_ef; e++)
    {
        ...
        hnswlib_set_ef(index, ef_search);
        // warmup
        for (int i = 0; i < 1000; i++)
        {
            float* vec = qv.data + (i % qv.n) * qv.dim;
            hnswlib_search(index, vec, K, &tempRset[0].ids);
        }
        ...
        for (int i = 0; i < qv.n; i++)
        {
            ...
            hnswlib_search(index, vec, K, &tempRset[i].ids);
        }
        timer_stop(&t);
...
int main()
{
    ...
    benchmark_searchhnswLib(3, "./benchmark/datasets/gist/gist");
